...
#define IR_THRESHOLD 30000   // tune if needed

// timing (ms)
const int STEP_DELAY   = 30;   // between LEDs in a sweep
const int PAUSE_SMALL  = 50;  // rest between small & medium
const int XTRA_SMALL_PAUSE = 30;
const int PAUSE_BIG    = 200;  // rest after big sweep
const int FADE_DELAY   = 30;   // ms between fade steps
const float FADE_MULT  = 0.85; // 0..1, lower = faster fade
const uint32_t SENSOR_START_DELAY_MS = 5000;  // 5 seconds

// bottom → top path you gave
int ledOrder[10] = {5, 4, 3, 6, 2, 9, 7, 8, 1, 0};
...
void setup() {
  ...
  waitWithOTA(SENSOR_START_DELAY_MS);
